...
import { SqlSettingsPanel } from "@/components/features/sql";
import { DatabaseExplorer, TableViewer, type DbObjectRef } from "@/components/features/sql/explorer";
import {
    Box,
    PanelLeftClose,
    PanelLeft,
...
export function SqlStudioLayout() {
    const [leftPanelOpen, setLeftPanelOpen] = useState(true);
    const [rightPanelOpen, setRightPanelOpen] = useState(true);
    const [leftTab, setLeftTab] = useState<"history" | "database">("history");
    const [mainTab, setMainTab] = useState<"chat" | "data">("chat");
    const [selection, setSelection] = useState<DbObjectRef | null>(null);
        ...
        <div className="h-screen w-full bg-background flex flex-col overflow-hidden">
            ...
            <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="p-2 border-b border-border">
                        <div className="flex items-center gap-1">
                            <button
                                ...
                                className={cn(
                                    "flex-1 rounded-md px-2 py-1.5 text-xs font-semibold uppercase tracking-wider",
                                    leftTab === "history" ? "bg-secondary text-foreground" : "text-muted-foreground hover:bg-secondary/60"
                                )}
                            >
                                History
                            </button>
                            <button
                                ...
                                className={cn(
                                    "flex-1 rounded-md px-2 py-1.5 text-xs font-semibold uppercase tracking-wider",
                                    leftTab === "database" ? "bg-secondary text-foreground" : "text-muted-foreground hover:bg-secondary/60"
                                )}
                            >
                                Database
                            </button>
                ...
                <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
                    <div className="h-10 shrink-0 border-b border-border flex items-center px-3 gap-2 bg-card">
                        <button
                            type="button"
                            onClick={() => setMainTab("chat")}
                            className={cn(
                                "rounded-md px-3 py-1.5 text-xs font-semibold uppercase tracking-wider",
                                mainTab === "chat" ? "bg-secondary text-foreground" : "text-muted-foreground hover:bg-secondary/60"
                            )}
                        >
                            Chat
                        </button>
                        <button
                            ...
                            className={cn(
                                "rounded-md px-3 py-1.5 text-xs font-semibold uppercase tracking-wider",
                                mainTab === "data" ? "bg-secondary text-foreground" : "text-muted-foreground hover:bg-secondary/60"
                            )}
                        >
                            Data
                        </button>
                        ...
                        <div className="ml-auto text-xs text-muted-foreground truncate">
                            {selection ? `${selection.schema}.${selection.name}` : "No selection"}
                        </div>
                    </div>

                    <div className="flex-1 min-w-0 overflow-hidden">
                        {mainTab === "chat" ? (
                            <ChatInterface toolScope="sql" />
                        ) : (
                            <TableViewer selection={selection} />
                        )}
                    </div>
                </main>
        ...
        return (
            <div className="flex items-center gap-1 p-1 bg-secondary rounded-md">
                <input
                    ...
                    onBlur={handleRename}
                />
                <button onClick={handleRename} className="p-1 rounded hover:bg-primary/20 text-primary">
                    <Check size={12} />
                </button>
                <button
                    onClick={() => {
    ...
    return (
        <div
            className={cn(
                ...
                isActive ? "bg-secondary text-foreground" : "hover:bg-secondary text-muted-foreground"
            )}
        >
            <button type="button" className="flex-1 text-left p-2.5 truncate" onClick={onSelect} title={conversation.title}>
                {conversation.title}
            </button>
            <div className={cn("absolute right-1 flex items-center gap-0.5", isActive ? "flex" : "hidden group-hover:flex")}>
                <button
                    type="button"
